...
function getDiffs(original, target) {
    for(var i = 0; i < original.groups.length; ++i) {
    original.groups[i].apps = _.sortBy(original.groups[i].apps, ['id']);
    }

    for(var i = 0; i < target.groups.length; ++i) {
        target.groups[i].apps = _.sortBy(target.groups[i].apps, ['id']);
    }

    original.groups = _.sortBy(original.groups, ['id']);
    target.groups = _.sortBy(target.groups, ['id']);

    const differences = diff(original, target)

    let changes = []
    _.forEach(differences, function(change) {
    
        if(_.indexOf(change.path, "version") > -1 ||
            _.indexOf(change.path, "versionInfo") > -1) {
            return;
        }
    
        let c2 = _.cloneDeep(change);
        let place = original;
        
        for(let i = 0; i < c2.path.length; ++i) {
            let step = c2.path[i]
    
            if ( typeof step === "number" ) {
                if ( _.has(place[step], 'id')) {
                    c2.path[i] = place[step].id
                }
            }
            place = place[step]
        }
        ...
        switch (event.type) {
            ...
            case "deployment_info":
                attachment = {
                    ...
                    "text": "The deployment `" + event.data.plan.id + "` triggered step `" + event.data.currentStep.actions[0].action + "` with the following changes:",
                    "fields": [],
                    "color": "#0066cc",
                    "mrkdwn_in": ["text", "fields"],
                    "ts": toUnixTimestamp(event.data.timestamp)
                };
                const changes = getDiffs(event.data.plan.original, event.data.plan.target)
